...
class Repository
  module Adapter
    class Rom
      ...
      def update(entity)
        mapper_for(entity.class).update entity
      end
      ...
      def mapper_for(klass)
        relation_name = klass.name.demodulize.underscore.pluralize
        ROM_SETUP.relations.send relation_name
      end
